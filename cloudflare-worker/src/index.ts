...
import { currentOpenAiRoutes } from "./routes/currentOpenAi";
import { functionRoutes } from "./routes/function";
import { adminCompatRoutes } from "./routes/adminCompat";
import { currentAdminRoutes } from "./routes/currentAdmin";
import { mediaRoutes } from "./routes/media";
import { adminRoutes } from "./routes/admin";
import { runKvDailyClear } from "./kv/cleanup";
import { ensureDbSchema } from "./schema";
import { getFaviconAssetPath, getStaticPagePath } from "./sitePaths";
...
async function fetchAsset(c: any, pathname: string): Promise<Response> {
  ...
  if (!assets) {
    ...
    return assetFetchError(
      'Internal Server Error: missing ASSETS binding. Check `wrangler.toml` `assets = { directory = "./.assets", binding = "ASSETS" }` and redeploy.',
      buildSha,
    );
  }
...
app.route("/", currentAdminRoutes);
app.route("/", adminCompatRoutes);
app.route("/v1", currentOpenAiRoutes);
...
app.get("/favicon.ico", (c) => fetchAsset(c, getFaviconAssetPath()));
...
for (const pathname of [
  ...
  app.get(pathname, (c) => {
    const buildSha = getBuildSha(c.env as Env);
    const v = c.req.query("v") ?? "";
    if (v !== buildSha) return c.redirect(`${pathname}?v=${encodeURIComponent(buildSha)}`, 302);
    const assetPath = getStaticPagePath(pathname);
